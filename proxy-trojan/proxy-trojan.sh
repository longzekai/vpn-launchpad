...
if [ -z "$DUCKDNSDOMAIN" ] || [ -z "$TRJPORT" ] || [ -z "$TRJPASS" ]; then
...
docker run --name $CTNNAME -p $SOCKSPORT:1080 -p $DNSPORT:53/udp -p $HTTPPORT:8123 -d $IMGNAME:$TARGET -h ${VHOST} -d ${DUCKDNSDOMAIN}.duckdns.org -p ${TRJPORT} -w ${TRJPASS} >/dev/null
echo "Done."
echo
